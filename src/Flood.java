...
public class Flood {
    public static void show(ArrayList<double[]> allData,double[][][] weights){
        ...
        for (int k = 0; k < allData.size(); k++) {
            ...
            for(int i = 0; i < denormOutput.length; i++){
                System.out.println("output = " + denormOutput[i] + " desired = " + (int)denormDesired[i]);
            }
    ...
    public static void main(String[] args) {
        ...
        double momentumRate = 0.99;
        double[][][] weights = Process.randomWeight(n_in, n_hidden, n_out);

        ArrayList<double[]> allData = FloodData.getAllData();
        Collections.shuffle(allData);
        ...
        double totalTestErrorAv = 0;
            ...
            for (int y = 0; y < folds; y++) {
                ...
                ArrayList<double[]> sample = FloodData.sample(trainSet, 1);
                ArrayList<double[]> desired_output = FloodData.desired_output(trainSet);
                ...
                for (int k = 0; k < trainSet.size(); k++) {
                    ...
                    double[] predictedOutput = forward.get(weights.length - 1);
                    ...
                    predictedOutput = forward.get(weights.length - 1);
                    for (int i = 1; i < predictedOutput.length; i++) {
                        lastOutput[i - 1] = predictedOutput[i];
                    }
                ...
                System.out.println("Fold " + (y + 1) + " - Average SSE at train set = " + sumTrainErrorAv);
                totalTrainErrorAv += sumTrainErrorAv;

                sample = FloodData.sample(testSet, 1);
                desired_output = FloodData.desired_output(testSet);
                ...
                System.out.println("Fold " + (y + 1) + " - Average SSE at test set = " + sumTestErrorAv);
