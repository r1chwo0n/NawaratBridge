...
public class FloodData {
    ...
    static public ArrayList<double[]> sample(ArrayList<double[]> allData, int bias){
        ...
        for(int i = 0; i < allData.size(); i ++){
            double[] input = new double[allData.get(0).length];
            input[0] = bias;
            for(int j = 1; j < input.length; j++){
                input[j] = (allData.get(i)[j-1] - min) / (max - min) ;
            }
            sample.add(input);
        }
        return sample;
    ...
    static public ArrayList<double[]> desired_output(ArrayList<double[]> allData){
        ...
        for(int i = 0; i < allData.size(); i++){
            ...
            input[0] = (allData.get(i)[8] - min) / (max - min);
            output.add(input);
        }
        return output;
    }

    public static void printAll(ArrayList<int[]> list) {
        for (int[] array : list) {
